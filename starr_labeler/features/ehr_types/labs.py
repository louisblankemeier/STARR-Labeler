...
class extract_labs(extract_base):
    ...
    def process_data(self, pat_data):
        ...
        pat_data.loc[:, "Reference Low"] = pd.to_numeric(
            pat_data.loc[:, "Reference Low"], "coerce"
        )
        ...
        pat_data.loc[~high_low_nan, "norm"] = (
            pat_data.loc[~high_low_nan, "Value"]
    ...
    def truncate_data(self, pat_data):
        ...
        pat_data = pat_data.loc[
            pat_data["Result"].str.contains(
                labs_regex, regex=True, case=False, na=False
            )
        ]
        ...
        for key, value in self.cfg["EHR_TYPES"]["LABS"]["REGEX_TO_FEATURE_NAME"].items():
            if value != "None":
                pat_data.loc[
                    pat_data["Result"].str.contains(
                        key, regex=True, case=False, na=False
                    ),
                    "Result",
                ] = value
        ...
        truncated.loc[
            truncated["Result"] == "eGFR Refit Without Race (2021)", "Reference Low"
        ] = 0
        truncated.loc[
            truncated["Result"] == "eGFR Refit Without Race (2021)", "Reference High"
        ] = 60
        repl = average
        truncated["Value"] = truncated["Value"].str.replace(
            r" *(\d) *- *(\d) *", repl, regex=True
        )
        truncated["Reference High"] = (
            truncated["Reference High"].astype(str).str.strip("<>= ")
        )
        truncated["Reference Low"] = (
            truncated["Reference Low"].astype(str).str.strip("<>= ")
        )
        truncated["Value"] = truncated["Value"].astype(str).str.strip("<>= ")
        truncated["Value"] = pd.to_numeric(truncated["Value"], errors="coerce")
        truncated["Reference High"] = pd.to_numeric(
            truncated["Reference High"], errors="coerce"
        )
        truncated["Reference Low"] = pd.to_numeric(
            truncated["Reference Low"], errors="coerce"
        )
