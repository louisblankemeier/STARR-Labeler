...
class extract_labs(extract_base):
    ...
    def process_data(self, pat_data):
        ...
        for key, value in self.cfg["FEATURES"]["TYPES"]["LABS"]["INCLUDE"].items():
            if value != "None":
                pat_data.loc[
                    pat_data["Result"].str.fullmatch(key, case=False, na=False), "Result"
                ] = value
        ...
        high_low_nan = np.logical_and(
            pd.isna(pat_data.loc[:, "Reference High"]), pd.isna(pat_data.loc[:, "Reference Low"])
        )
    ...
    def truncate_data(self, pat_data):
        ...
        for key, value in self.cfg["FEATURES"]["TYPES"]["LABS"]["INCLUDE"].items():
            if value != "None":
                pat_data.loc[
                    pat_data["Result"].str.contains(key, regex=True, case=False, na=False), "Result"
                ] = value
