...
class extract_base:
    ...
    def pivot(self, merged):
        input_features = merged.loc[
            (merged.Imaging_dt - merged.Event_dt).dt.days > (-1 * self.lag_after_dates)
        ]
        ...
        if self.aggregate == "pce":
            ...
            for i in range(1, self.bins):
                input_features.loc[
                    (input_features.Imaging_dt - input_features.Event_dt).dt.days
                    > (365 * i * self.bin_duration),
                    "Period",
                ] = str(i + 1)
    ...
    def process_type(self, rows=1000000, fillna="none"):
        ...
        lab_input_features = all_mrn_accession.merge(
            lab_input_features, how="left", on=["Patient Id", "Accession Number"]
        )
        if fillna != "none":
            if fillna == "median":
                lab_input_features = lab_input_features.fillna(
                    lab_input_features.median(numeric_only=True)
                )
            else:
                lab_input_features = lab_input_features.fillna(float(fillna))

        # make the data frame behave well for use with XGBoost
        lab_input_features["Patient Id"] = (
            lab_input_features["Patient Id"]
            .astype("string")
            .str.replace(r"^(0+)", "", regex=True)
            .fillna("0")
        )
        lab_input_features["Accession Number"] = (
            lab_input_features["Accession Number"]
            .astype("string")
            .str.replace(r"^(0+)", "", regex=True)
            .fillna("0")
        ...
        # lab_input_features.columns = [regex.sub("_", col) if any(x in str(col)
        # for x in set(('[', ']', '<')))
        # else col for col in lab_input_features.columns.values]
