...
def visualize_progression(pat_data, lab_tests, pat_id, ct_dates = None):
    ...
    plt.bar(list(pat_data['Type'].value_counts().index[0:20]), pat_data['Type'].value_counts()[0:20])
    ...
    for i in range(5):
        for j in range(4):
            ...
            fig_num += 1
            axs[i, j].plot_date(list(pat_data.loc[pat_data.Type == lab_test]['Event_dt']), list(pat_data.loc[pat_data.Type == lab_test]['Value']), color = 'b')
...
def merge_dfms(dfs):
    if len(dfs) == 1:
        ...
        for i in range(len(dfs) - 1):
            if i == 0:
                input_features = dfs[i].merge(dfs[i + 1], how = 'outer', on = ['Patient Id', 'Accession Number'])
            else:
                input_features = input_features.merge(dfs[i + 1], how = 'outer', on = ['Patient Id', 'Accession Number'])
...
def data_iterator(base_path, file_name, use_cols, chunksize = 1000000):
    ...
    if file_name in directories:
        iterators.append(pd.read_csv(os.path.join(base_path, file_name), chunksize = chunksize, usecols = use_cols, low_memory = False))
    else:
        for dir_name in [str(1), str(2), str(3)]:
            iterators.append(pd.read_csv(os.path.join(base_path, dir_name, file_name), chunksize = chunksize, usecols = use_cols, low_memory = False))
...
class patient_iterator():
    ...
    def __iter__(self):
        self.patient_idx = 0
        self.my_data_iterator = iter(data_iterator(self.base_path, self.file_name, self.use_cols, self.chunksize))
    ...
    def __next__(self): 
        if self.stop:
            raise StopIteration
        reserve_chunk = self.curr_data_chunk.loc[self.curr_data_chunk['Patient Id'] == self.unique_ids[-1]]
